#Anthony Windmon - MLP Classifier
import csv
import numpy
import pandas as pd
...
X_train, X_test, y_train, y_test = train_test_split(df.drop(columns=['Class']), binary_labels, test_size=0.25)
...
#MLP model
model = MLPClassifier(solver='lbfgs', alpha=1e-5, hidden_layer_sizes=(5,2), activation = 'relu',
...
X_train, X_test, y_train, y_test = train_test_split(df.drop(columns=['Class']), binary_labels, test_size=0.45,
                                                    random_state=42)
print("The length of the training set =", len(X_train)) #training sample
print("The length of the testing set =", len(X_test)) #testing sample

#MLP Classification Test 2
model_two = model =  MLPClassifier(solver='lbfgs', alpha=1e-5, hidden_layer_sizes=(8,3), activation = 'tanh',
                        learning_rate = 'constant', learning_rate_init = 0.001, max_iter=215, momentum=0.35,
                            batch_size = 'auto', beta_1=0.9, beta_2=0.99, early_stopping=False,random_state=1,
                                power_t=0.5, validation_fraction=0.1)
...
plt.title('ROC of Before & After Breath Using MLP')
